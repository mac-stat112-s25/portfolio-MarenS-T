...
different capitalizations, different spellings
...
black and white, color, and N/A; unknown due to different category names
...
imdb_messy |>
 colSums(is.na(actor_1_facebook_likes)))
```
...
```
They seem to all be documentaries so there aren't actors
...
```{r}
imdb_temp <- imdb_messy |>
  mutate(actor_1_facebook_1 = replace_na(actor_1_facebook_likes, 0))
```
